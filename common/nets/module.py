...
class PositionNet(nn.Module):
    def __init__(self):
        ...
        self.conv = make_conv_layers(
            [2048, self.joint_num * self.depth_dim], kernel=1, stride=1, padding=0, bnrelu_final=False
        )
...
class RotationNet(nn.Module):
    def __init__(self):
        ...
        self.pose_out = make_linear_layers(
            [self.joint_num * (512 + 3), (mano.orig_joint_num - 1) * 6], relu_final=False
        )  # without root joint
...
class TransNet(nn.Module):
    ...
    def forward(self, rjoint_img, ljoint_img, rhand_bbox, lhand_bbox):
        rjoint_img, ljoint_img, rhand_bbox, lhand_bbox = (
            rjoint_img.clone(),
            ljoint_img.clone(),
            rhand_bbox.clone(),
            lhand_bbox.clone(),
        ...
        rhand_hm = rhand_hm.view(
            -1, self.joint_num * cfg.input_hm_shape[0], cfg.input_hm_shape[1], cfg.input_hm_shape[2]
        )
        lhand_hm = self.render_gaussian_heatmap(ljoint_img)
        lhand_hm = lhand_hm.view(
            -1, self.joint_num * cfg.input_hm_shape[0], cfg.input_hm_shape[1], cfg.input_hm_shape[2]
        )
        ...
        wrist_img = torch.stack(
            (rjoint_img[:, mano.sh_root_joint_idx, :], ljoint_img[:, mano.sh_root_joint_idx, :]), 1
        )
