...
class MANO(object):
    def __init__(self):
        ...
        self.layer = {
            "right": smplx.create(
                cfg.human_model_path, "mano", is_rhand=True, use_pca=False, flat_hand_mean=False, **self.layer_arg
            ),
            "left": smplx.create(
                cfg.human_model_path, "mano", is_rhand=False, use_pca=False, flat_hand_mean=False, **self.layer_arg
            ),
        }
        self.vertex_num = 778
        ...
        self.sh_joint_regressor = transform_joint_to_other_db(
            self.orig_joint_regressor, self.orig_joints_name, self.sh_joints_name
        )
        self.sh_joint_regressor[self.sh_joints_name.index("Thumb_4")] = np.array(
            [1 if i == 745 else 0 for i in range(self.sh_joint_regressor.shape[1])], dtype=np.float32
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Index_4")] = np.array(
            [1 if i == 317 else 0 for i in range(self.sh_joint_regressor.shape[1])], dtype=np.float32
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Middle_4")] = np.array(
            [1 if i == 445 else 0 for i in range(self.sh_joint_regressor.shape[1])], dtype=np.float32
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Ring_4")] = np.array(
            [1 if i == 556 else 0 for i in range(self.sh_joint_regressor.shape[1])], dtype=np.float32
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Pinky_4")] = np.array(
            [1 if i == 673 else 0 for i in range(self.sh_joint_regressor.shape[1])], dtype=np.float32
        ).reshape(1, -1)
        ...
        self.th_root_joint_idx = {
            "right": self.th_joints_name.index("R_Wrist"),
            "left": self.th_joints_name.index("L_Wrist"),
        }
        self.th_flip_pairs = [(i, i + 21) for i in range(21)]
        self.th_joint_type = {
            "right": np.arange(0, self.th_joint_num // 2),
