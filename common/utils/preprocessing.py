...
def process_db_coord(
    ...
    joint_img, joint_cam, joint_valid, rel_trans = (
        joint_img.copy(),
        joint_cam.copy(),
        joint_valid.copy(),
        rel_trans.copy(),
    ...
    joint_img[:, 2] = (joint_img[:, 2] / (cfg.bbox_3d_size / 2) + 1) / 2.0 * cfg.output_body_hm_shape[0]

    # check truncation
    joint_trunc = joint_valid * (
        (joint_img[:, 0] >= 0)
        * (joint_img[:, 0] < cfg.output_body_hm_shape[2])
        * (joint_img[:, 1] >= 0)
        * (joint_img[:, 1] < cfg.output_body_hm_shape[1])
        * (joint_img[:, 2] >= 0)
        * (joint_img[:, 2] < cfg.output_body_hm_shape[0])
