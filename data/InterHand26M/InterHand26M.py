...
from common.utils.vis import save_obj, vis_3d_skeleton, vis_keypoints, vis_mesh
...
class InterHand26M(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.joint_set["joint_type"] = {
            ...
            "left": np.arange(self.joint_set["joint_num"] // 2, self.joint_set["joint_num"]),
        }
    ...
    def load_data(self):
        ...
        with open(
            osp.join(self.annot_path, self.data_split, "InterHand2.6M_" + self.data_split + "_camera.json")
        ) as f:
            cameras = json.load(f)
        with open(
            osp.join(self.annot_path, self.data_split, "InterHand2.6M_" + self.data_split + "_joint_3d.json")
        ) as f:
            joints = json.load(f)
        with open(
            osp.join(self.annot_path, self.data_split, "InterHand2.6M_" + self.data_split + "_MANO_NeuralAnnot.json")
        ) as f:
        ...
        for aid in aid_list:
            ...
            joint_valid = np.array(joints[str(capture_id)][str(frame_idx)]["joint_valid"], dtype=np.float32).reshape(
                -1, 1
            )
            ...
            # joint coordinates
            joint_world = np.array(joints[str(capture_id)][str(frame_idx)]["world_coord"], dtype=np.float32).reshape(
                -1, 3
            )
            joint_cam = world2cam(joint_world, R, t)
            joint_cam[np.tile(joint_valid == 0, (1, 3))] = 1.0  # prevent zero division error
            ...
            if np.sum(joint_trunc[self.joint_set["joint_type"]["left"]]) == 0:
                ...
                lhand_bbox = get_bbox(
                    joint_img[self.joint_set["joint_type"]["left"], :2],
            ...
            if np.sum(joint_trunc[self.joint_set["joint_type"]["right"]]) == 0:
                ...
                rhand_bbox = get_bbox(
                    joint_img[self.joint_set["joint_type"]["right"], :2],
    ...
    def __getitem__(self, idx):
        ...
        joint_img, joint_cam, joint_valid, joint_trunc, rel_trans = process_db_coord(
            joint_img,
            joint_cam,
            joint_valid,
            rel_trans,
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            if vis:
                ...
                img = cv2.rectangle(
                    img.copy(),
                    (int(lhand_bbox[0]), int(lhand_bbox[1])),
                    (int(lhand_bbox[2]), int(lhand_bbox[3])),
                    (255, 0, 0),
                    3,
                )
                ...
                img = cv2.rectangle(
                    img.copy(),
                    (int(rhand_bbox[0]), int(rhand_bbox[1])),
                    (int(rhand_bbox[2]), int(rhand_bbox[3])),
                    (0, 0, 255),
                    3,
                )
                ...
                save_obj(
                    out["lmano_mesh_cam"] + out["rel_trans"].reshape(1, 3), mano.face["left"], filename + "_left.obj"
                )
            ...
            for h in ("right", "left"):
                ...
                mesh_gt[vertex_mask, :] = (
                    mesh_gt[vertex_mask, :]
                    - np.dot(mano.sh_joint_regressor, mesh_gt[vertex_mask, :])[mano.sh_root_joint_idx, None, :]
                )
                mesh_out[vertex_mask, :] = (
                    mesh_out[vertex_mask, :]
                    - np.dot(mano.sh_joint_regressor, mesh_out[vertex_mask, :])[mano.sh_root_joint_idx, None, :]
                )
            ...
            if annot["hand_type"] == "right" and annot["mano_param"]["right"] is not None:
                eval_result["mpvpe_sh"][n] = np.sqrt(
                    np.sum((mesh_gt[: mano.vertex_num, :] - mesh_out[: mano.vertex_num, :]) ** 2, 1)
                ).mean()
