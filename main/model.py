...
from common.utils.transforms import restore_bbox, rot6d_to_axis_angle
from main.config import cfg


class Model(nn.Module):
    def __init__(
        self, body_backbone, body_box_net, hand_roi_net, hand_position_net, hand_rotation_net, hand_trans_net
    ):
            ...
            torch.FloatTensor(
                [
                    math.sqrt(
                        ...
                        * cfg.camera_3d_size
                        * cfg.camera_3d_size
                        / (cfg.input_hand_shape[0] * cfg.input_hand_shape[1])
                    )
                ]
            )
            .cuda()
    ...
    def get_coord(self, root_pose, hand_pose, shape, cam_trans, hand_type):
        ...
        joint_cam = torch.bmm(
            torch.from_numpy(mano.sh_joint_regressor).cuda()[None, :, :].repeat(batch_size, 1, 1), mesh_cam
        )
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        )  # (2N, ...). flipped left hand + right hand

        # hand network
        joint_img = self.hand_position_net(hand_feat)  # (2N, J_P, 3)
        mano_root_pose, mano_hand_pose, mano_shape, cam_trans = self.forward_rotation_net(
            hand_feat, joint_img.detach()
        )
        lhand_num = len(lhand_bbox)
        rhand_num = len(rhand_bbox)
        ...
        # get outputs
        ljoint_proj, ljoint_cam, lmesh_cam, lroot_cam = self.get_coord(
            lroot_pose, lhand_pose, lshape, lcam_trans, "left"
        )
        rjoint_proj, rjoint_cam, rmesh_cam, rroot_cam = self.get_coord(
            rroot_pose, rhand_pose, rshape, rcam_trans, "right"
        )

        # relative translation
        rel_trans = self.hand_trans_net(
            rjoint_img.detach(), ljoint_img.detach(), rhand_bbox.detach(), lhand_bbox.detach()
        )
        ...
        joint_cam_learn = self.joint_regressor(mesh_cam.detach().view(-1, mano.vertex_num * 3)).view(
            -1, mano.sh_joint_num, 3
        )  # detach mesh to prevent gradients from back propagating
        ljoint_cam_learn = joint_cam_learn[:lhand_num, :, :]
        ljoint_cam_learn = torch.cat(
            (-ljoint_cam_learn[:, :, 0:1], ljoint_cam_learn[:, :, 1:]), 2
        )  # flip back to the left hand
        ...
        if mode == "train":
            ...
            if not cfg.use_gt_hand_bbox:
                loss["lhand_bbox_center"] = self.coord_loss(
                    lhand_bbox_center, targets["lhand_bbox_center"], meta_info["lhand_bbox_valid"][:, None]
                )
                loss["lhand_bbox_size"] = self.coord_loss(
                    lhand_bbox_size, targets["lhand_bbox_size"], meta_info["lhand_bbox_valid"][:, None]
                )
                loss["rhand_bbox_center"] = self.coord_loss(
                    rhand_bbox_center, targets["rhand_bbox_center"], meta_info["rhand_bbox_valid"][:, None]
                )
                loss["rhand_bbox_size"] = self.coord_loss(
                    rhand_bbox_size, targets["rhand_bbox_size"], meta_info["rhand_bbox_valid"][:, None]
                )
            loss["rel_trans"] = self.coord_loss(
                rel_trans[:, None, :], targets["rel_trans"][:, None, :], meta_info["rel_trans_valid"][:, None, :]
            )
            ...
            loss["joint_cam"] = (
                self.coord_loss(
                    joint_cam, targets["joint_cam"], meta_info["joint_valid"] * meta_info["is_3D"][:, None, None]
                )
                * 10
            )
            loss["joint_cam_learn"] = (
                self.coord_loss(
                    joint_cam_learn, targets["joint_cam"], meta_info["joint_valid"] * meta_info["is_3D"][:, None, None]
                )
                * 10
            )
            loss["mano_joint_cam"] = (
                self.coord_loss(joint_cam, targets["mano_joint_cam"], meta_info["mano_joint_valid"]) * 10
            )
            ...
            for part_name, bbox in (("left", lhand_bbox), ("right", rhand_bbox)):
                for coord_name, trunc_name in (("joint_img", "joint_trunc"), ("mano_joint_img", "mano_joint_trunc")):
                    ...
                    x *= cfg.output_hand_hm_shape[2] / (
                        (bbox[:, None, 2] - bbox[:, None, 0]) / cfg.input_body_shape[1] * cfg.output_body_hm_shape[2]
                    )
                    y -= bbox[:, None, 1] / cfg.input_body_shape[0] * cfg.output_body_hm_shape[1]
                    y *= cfg.output_hand_hm_shape[1] / (
                        (bbox[:, None, 3] - bbox[:, None, 1]) / cfg.input_body_shape[0] * cfg.output_body_hm_shape[1]
                    )
                    z *= cfg.output_hand_hm_shape[0] / cfg.output_body_hm_shape[0]
                    trunc *= (
                        (x >= 0) * (x < cfg.output_hand_hm_shape[2]) * (y >= 0) * (y < cfg.output_hand_hm_shape[1])
                    )
                    ...
                    targets[coord_name] = torch.cat(
                        (
                            targets[coord_name][:, : mano.th_joint_type[part_name][0], :],
                            coord,
                            targets[coord_name][:, mano.th_joint_type[part_name][-1] + 1 :, :],
                        ),
                        1,
                    )
                    meta_info[trunc_name] = torch.cat(
                        (
                            meta_info[trunc_name][:, : mano.th_joint_type[part_name][0], :],
                            trunc,
                            meta_info[trunc_name][:, mano.th_joint_type[part_name][-1] + 1 :, :],
                        ),
                        1,
                    )

            loss["joint_img"] = self.coord_loss(
                joint_img, targets["joint_img"], meta_info["joint_trunc"], meta_info["is_3D"]
            )
